...
class LoginWithIdDatasourceImplV2 implements LoginWithIdDatasource {
  ...
  Future<LoggedUser> call(LoginWithIdParams params) async {
    ...
    try {
      ...
      result.fold(
        (exception) {
          // Verifica se a mensagem é de email não confirmado
          if (exception.message.contains('Email not confirmed')) {
            throw const IncorrectCredentialsException(
              message:
                  'Sua conta de e-mail não está confirmada. Um link de confirmação foi enviado para o seu e-mail cadastrado.',
              stackTrace: StackTrace.empty,
            );
          }

          // Se a mensagem contém 400, trata como credenciais incorretas
          if (exception.message.contains('400')) {
            throw IncorrectCredentialsException(
              message: 'login_with_id_datasource_incorrect_credentials'.i18n(),
              stackTrace: StackTrace.empty,
            );
          }
        },
