...
class CameraExampleController extends StreamStore<Exception, CameraStatus> {
  CameraExampleController() : super(const Initial(photos: [])) {
    setLoading(true);
  ...
  void excludeCurrentPhotoAndTakeAnother() {
    ...
    final currentStateWithoutItsLastTakenPhoto = state.copyWith(photos: oldPhotoList..removeLast());
    update((currentStateWithoutItsLastTakenPhoto as TakenPhoto).alternative);
  }
  ...
  void _initializeCamera() {
    availableCameras().then((returnedCameras) async {
      ...
      final controller = CameraController(returnedCameras[0], ResolutionPreset.max, enableAudio: false);
