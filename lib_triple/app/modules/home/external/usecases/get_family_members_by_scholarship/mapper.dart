  ...
  Entity fromListToEntity(dynamic results) {
    ...
    for (final result in results) {
      if (!result.keys.contains('name')) {
        throw const MapperException(
            message:
                'Não foi possível recuperar a bolsa: Campo name não encontrado');
      }
      ...
      if (name is! String) {
        throw MapperException(
            message:
                'Não foi possível recuperar a bolsa: Campo name é inesperado (${name.runtimeType})');
      }
      if (name.isEmpty) {
        throw const MapperException(
            message: 'Não foi possível recuperar a bolsa: Campo name é vazio');
      }
      ...
      if (!result.keys.contains('isResponsible')) {
        throw const MapperException(
            message:
                'Não foi possível recuperar a bolsa: Campo responsável não encontrado');
      }

      final isResponsible = result['isResponsible'];

      if (isResponsible is! bool) {
        throw const MapperException(
            message:
                'Não foi possível recuperar a bolsa: Campo responsável é inesperado');
      }
